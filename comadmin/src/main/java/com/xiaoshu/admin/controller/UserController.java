...
public class UserController {
    ...
    @ResponseBody
    public ResponseEntity uploadFile1(@RequestParam("icon") MultipartFile file, HttpServletRequest httpServletRequest) {
        boolean x;
    	if(file == null){
            return ResponseEntity.failure("上传文件为空 ");
        }
        ...
        try {
        	...
        	String path = "/"+user.getLoginName()+"/"+file.getOriginalFilename();
        	FtpHandler ftp = readConfig.ftpHandler();
        	x = ftp.createDirImg(user.getLoginName());
        	user.setImgPath(path);
/*            url = uploadService.upload(file, path);*/
        	if(x){
        		uploadService.uploadFtp(file, path);
        		userService.updateUser(user);
        	}else{
        		return ResponseEntity.failure("创建文件夹失败，请查看FTP是否连接正确 ");
        	}
            map.put("url", url);
            map.put("name", file.getOriginalFilename());
        } catch (Exception e) {
